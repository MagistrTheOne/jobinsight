...
import { optimizeForImpact } from '@/lib/advanced-ats-analysis';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!jobDescription || jobDescription.trim().length < 50) {
      return NextResponse.json(
        { error: 'Job description is required for impact optimization' },
        { status: 400 }
      );
    }

    const optimization = await optimizeForImpact(resumeContent, jobDescription);
    ...
    console.error('Impact optimization error:', error);
    return NextResponse.json(
      {
        success: false,
        error: 'Impact optimization failed',
